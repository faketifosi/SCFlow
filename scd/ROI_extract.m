...
% mkdir( strcat('test sample\invoice image\saveas\', num2str(file) ) );
% 建立與檔案編號相同的存檔目錄

% 初次旋轉
totalRotAngle = 0;      %累計當前 ROI 對原圖總共的旋轉角度

im_center_x = size(im,2) / 2;   %目標影像的座標中心
im_center_y = size(im,1) / 2;

theta = atand( (candidate_ROI(2) - candidate_ROI(6) ) / ( candidate_ROI(1) - candidate_ROI(5) ) );      % 計算ROI相對於水平的傾斜角度
...
rot_im_center_y = size(rot_im,1) / 2;
    ...
    while iterative
        ...
        % 存出某個迭代階段在原影像中的ROI狀態
        %{                 
        print( fh, strcat('test sample\invoice image\saveas\', num2str(file) ,'\ori-step', num2str(step), '.eps'), '-depsc' );
        saveas(fh, strcat('test sample\invoice image\saveas\', num2str(file) ,'\ori-step', num2str(step), '.jpg') );
        %} 
        
        % 在旋轉後的影像上顯示目前取得的 ROI
        fh = figure(4);
        imshow(rot_im, 'border', 'tight', 'initialmagnification', 'fit');
        ...
        %{       
        print( fh, strcat('test sample\invoice image\saveas\', num2str(file) ,'\rot-step', num2str(step), '.eps'), '-depsc' );
        saveas(fh, strcat('test sample\invoice image\saveas\', num2str(file) ,'\rot-step', num2str(step), '.jpg') );
        %}    
       ...
       %
        ...
        BW_ROI = im2bw(BW_ROI, graythresh( ROI(find(map==128)) ));  % Otsu 二值化      
        ...
        if size(CCp,1) > 1
            for i = size(CCp,1):-1:1
                if CCp(i).Area < 5 || CCp(i).BoundingBox(3) / CCp(i).BoundingBox(4) > 1.5
        ...
        %{ 
        初始化最大 component 變數 (作為之後 ROI之篩選.擴張.移動.旋轉 之參考目標) (已改為找前75%大的size 為參考對象 以控制干擾與雜訊)
        ...
        maxAreaCC = index( floor(size(index,1)*0.75) + 1 );
        ...
        quantity = 0;    % number of character
        skewErrTop = 0; skewErrBot = 0;     % 累積傾斜誤差

        % 以中等面積 component 為參考篩選大小過小的 component
        % 並計算各別 component 對於最大 component 的上下高度累計誤差, 做為評估旋轉之用
        for i = size(CCp,1):-1:1
            area = CCp(i).BoundingBox(3) * CCp(i).BoundingBox(4);
            if abs(area - maxArea) < maxArea*0.6  %> maxArea*0.4
        ...
        set(fh,'Position',[1463,600, size(map,2)*1, size(map,1)*1]);
        
        %{    
        print( fh, strcat('test sample\invoice image\saveas\', num2str(file) ,'\map-step', num2str(step), '.eps'), '-depsc' );
        saveas(fh, strcat('test sample\invoice image\saveas\', num2str(file) ,'\map-step', num2str(step), '.jpg') );
        %}
        
            
        
       %% false positive elimination　（ROI狀態與參考對象比例超出容許範圍則停止）
        if (size(ROI,2) / maxAreaCC_struct.BoundingBox(3) > ROI_property.ROI_to_CCs_ratio.widthRatio * 3.5) ...          
                || (size(ROI,1) / maxAreaCC_struct.BoundingBox(4) > ROI_property.ROI_to_CCs_ratio.heightRatio * 7) ...   
       ...
       %% rotate              
        ...
        if skewErrTop >= maxAreaCC_struct.BoundingBox(4)*0.6 && skewErrBot >= maxAreaCC_struct.BoundingBox(4)*0.6 && ~rotated && rotate_count < 10 %限制次數上限
            ...
            if adjustAngle ~= 0 && abs(adjustAngle) ~= 90 %　若評估旋轉角不為0，則旋轉並設定迭代
       ...
       %% expand. shift 
        ...
        if quantity < number_of_object % component 小於8個 向左或向右擴張
            ...
            % 最開始的做法，參考 map 的 component 往兩側空白較少處擴展
            %{
            if ( ceil(CCp(1).BoundingBox(1)) - 1 ) <= ( 1.5*maxAreaCC_struct.BoundingBox(3) )
                ROI_rect_ori(1) = ROI_rect_ori(1) - maxAreaCC_struct.BoundingBox(3);
                ROI_width = ROI_width + maxAreaCC_struct.BoundingBox(3);
                iterative = true;
            else
                ROI_rect_ori(1) = ROI_rect_ori(1) + maxAreaCC_struct.BoundingBox(3);
                doShift = true;
            ...
            if ( ROI_width - ceil(CCp(end).BoundingBox(1) + CCp(end).BoundingBox(3))) <= ( 1.5*maxAreaCC_struct.BoundingBox(3) ) && ~iterative
                ROI_width = ROI_width + maxAreaCC_struct.BoundingBox(3);
                iterative = true;
            elseif ~doShift && ~iterative
                ROI_rect_ori(1) = ROI_rect_ori(1) - maxAreaCC_struct.BoundingBox(3);
                iterative = true;
            end
            %}
        ...
        % 儲存目前 ROI 內容
        %{     
        print( fh, strcat('test sample\invoice image\saveas\', num2str(file) ,'\ROI-step', num2str(step), '.eps'), '-depsc' );
        saveas(fh, strcat('test sample\invoice image\saveas\', num2str(file) ,'\ROI-step', num2str(step), '.jpg') );
        %}   
    ...
    for i = 1:size(CCp,1)
        ...
        imwrite( character , strcat('test sample\banknote image\saveas\characters\', num2str(file), '_', num2str(i) , '.jpg') );
    end
    %}
